#ifndef ARRAYLIST_HPP              // يعني لو شملنا ArrayList.hpp مرتين في نفس المشروع مايصير خطاء
#define ARRAYLIST_HPP             //تعريف الحماية لمنع التضمين المزدوج

#include "EmptyList.hpp"               //لما نحاول الوصول إلى أول عنصر وقائمة فاضية.
#include "NoSuchElement.hpp"           //لما نبحث عن عنصر غير موجود.
#include "IndexOutOfBound.hpp"        //لما نحاول الوصول إلى فهرس خارج حدود القائمة.

template <typename Object>              //فهي قالب عام (Template) يسمح باستخدامها مع أي نوع بيانات:
                                         //مثل ArrayList<int>, ArrayList<string> أو حتى ArrayList<Person>.                       
                                         
   class ArrayList{              // يعني الكلاس قابل لاستخدام أي نوع بيانات     
	private:
		Object *items;          // items: مؤشّر لمصفوفة ديناميكية (تخزن العناصر).
		int length;             //length: عدد العناصر الحالية داخل القائمة.
		int capacity;          //capacity: الحجم الفعلي للمصفوفة المحجوزة في الذاكرة
...
#endif  // التضمين المزدوج هنا يغلق 